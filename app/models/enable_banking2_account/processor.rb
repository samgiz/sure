class EnableBanking2Account::Processor
  ...
  def process
    unless enable_banking2_account.current_account.present?
      Rails.logger.info "EnableBanking2Account::Processor - No linked account for enable_banking2_account #{enable_banking2_account.id}, skipping processing"
      return
    end

    Rails.logger.info "EnableBanking2Account::Processor - Processing enable_banking2_account #{enable_banking2_account.id} (uid #{enable_banking2_account.uid})"

    begin
      process_account!
    rescue StandardError => e
      Rails.logger.error "EnableBanking2Account::Processor - Failed to process account #{enable_banking2_account.id}: #{e.message}"
  ...
  private
    ...
    def process_account!
      if enable_banking2_account.current_account.blank?
        Rails.logger.error("Enable Banking account #{enable_banking2_account.id} has no associated Account")
        return
      end

      account = enable_banking2_account.current_account
      balance = enable_banking2_account.current_balance

      # A nil current_balance means this sync's balance fetch did not succeed:
      # upsert_enable_banking2_snapshot! clears it and only a successful
      ...
      if balance.nil?
        Rails.logger.warn("EnableBanking2Account::Processor - No balance available for enable_banking2_account #{enable_banking2_account.id} (balance fetch failed or not yet run), skipping account update")
        return
      end
      ...
      if account.accountable_type == "Loan" || account.accountable_type == "CreditCard"
        ...
        if account.accountable_type == "CreditCard"
          if enable_banking2_account.credit_limit.present?
            # Compute available credit based on the strictly positive outstanding debt
            available = enable_banking2_account.credit_limit - outstanding_debt
            available_credit = [ available, 0 ].max
            unless account.accountable.present?
              Rails.logger.warn "EnableBanking2Account::Processor - CreditCard accountable missing for account #{account.id}"
            end
      ...
      currency = parse_currency(enable_banking2_account.currency) || account.currency || "EUR"
    ...
    def process_transactions
      EnableBanking2Account::Transactions::Processor.new(enable_banking2_account).process
    rescue => e
    ...
    def report_exception(error, context)
      Sentry.capture_exception(error) do |scope|
        scope.set_tags(
          enable_banking2_account_id: enable_banking2_account.id,
          context: context
        )
      end
    end
