class EnableBanking2Item < ApplicationRecord
  ...
  has_many :enable_banking2_accounts, dependent: :destroy
  has_many :accounts, through: :enable_banking2_accounts
  ...
  def start_authorization(aspsp_name:, redirect_url:, state: nil, psu_type: "personal",
    ...
    provider = enable_banking2_provider
  ...
  def reconcile_session_expiry!(session_data)
    ...
    Rails.logger.warn "EnableBanking2Item #{id} - Failed to reconcile session expiry: #{e.message}"
  end

  def import_latest_enable_banking2_data
    provider = enable_banking2_provider
    unless provider
      Rails.logger.error "EnableBanking2Item #{id} - Cannot import: Enable Banking provider is not configured"
      raise StandardError.new("Enable Banking provider is not configured")
    end

    unless session_valid?
      Rails.logger.error "EnableBanking2Item #{id} - Cannot import: Session is not valid"
      update!(status: :requires_update)
      raise StandardError.new("Enable Banking session is not valid or has expired")
    end

    EnableBanking2Item::Importer.new(self, enable_banking2_provider: provider).import
  rescue => e
    Rails.logger.error "EnableBanking2Item #{id} - Failed to import data: #{e.message}"
    raise
  end

  def process_accounts
    return [] if enable_banking2_accounts.empty?

    results = []
    enable_banking2_accounts.joins(:account).merge(Account.visible).each do |enable_banking2_account|
      begin
        result = EnableBanking2Account::Processor.new(enable_banking2_account).process
        results << { enable_banking2_account_id: enable_banking2_account.id, success: true, result: result }
      rescue => e
        Rails.logger.error "EnableBanking2Item #{id} - Failed to process account #{enable_banking2_account.id}: #{e.message}"
        results << { enable_banking2_account_id: enable_banking2_account.id, success: false, error: e.message }
      end
    end
  ...
  def schedule_account_syncs(parent_sync: nil, window_start_date: nil, window_end_date: nil)
    ...
    accounts.visible.each do |account|
      begin
        ...
        Rails.logger.error "EnableBanking2Item #{id} - Failed to schedule sync for account #{account.id}: #{e.message}"
  ...
  def upsert_enable_banking2_snapshot!(accounts_snapshot)
    assign_attributes(
  ...
  def linked_accounts_count
    enable_banking2_accounts.joins(:account_provider).count
  end

  def unlinked_accounts_count
    enable_banking2_accounts.left_joins(:account_provider).where(account_providers: { id: nil }).count
  end

  def total_accounts_count
    enable_banking2_accounts.count
  end
  ...
  def sync_status_summary
    ...
    if latest.sync_stats.present?
      ...
      total_accounts = enable_banking2_accounts.count
  ...
  def connected_institutions
    enable_banking2_accounts.includes(:account)
  ...
  def revoke_session
    ...
    begin
      ...
      Rails.logger.warn "EnableBanking2Item #{id} - Failed to revoke session: #{e.message}"
    ensure
      update!(
        session_id: nil,
  ...
  private
    ...
    def fetch_aspsp_data(aspsp_name)
      ...
      Rails.logger.warn "EnableBanking2Item #{id} - could not fetch ASPSP metadata for #{aspsp_name}: #{e.message}"
      nil
    end
    ...
    def parse_session_expiry(session_result)
      ...
      Rails.logger.warn "EnableBanking2Item #{id} - Failed to parse session expiry: #{e.message}"
      90.days.from_now
    end
    ...
    def import_accounts_from_session(accounts_data)
      ...
      accounts_data.each do |account_data|
        ...
        enable_banking2_account = enable_banking2_accounts.find_or_initialize_by(uid: uid)
