...
class Provider::EnableBanking2
  ...
  def get_aspsps(country:)
    ...
    raise EnableBanking2Error.new("Exception during GET request: #{e.message}", :request_failed)
  end
  ...
  def start_authorization(aspsp_name:, aspsp_country:, redirect_url:, state: nil,
    ...
    raise EnableBanking2Error.new("Exception during POST request: #{e.message}", :request_failed)
  end
  ...
  def delete_session(session_id:)
    ...
    raise EnableBanking2Error.new("Exception during DELETE request: #{e.message}", :request_failed)
  end
  ...
  private
    ...
    def extract_private_key(certificate_pem)
      ...
      raise EnableBanking2Error.new("Invalid private key in certificate: #{e.message}", :invalid_certificate)
    end
    ...
    def handle_response(response)
      ...
      when 204
        {}
      when 400
        raise EnableBanking2Error.new("Bad request to Enable Banking API: #{response.body}", :bad_request)
      when 401
        raise EnableBanking2Error.new("Invalid credentials or expired JWT", :unauthorized)
      when 403
        raise EnableBanking2Error.new("Access forbidden - check your application permissions", :access_forbidden)
      when 404
        raise EnableBanking2Error.new("Resource not found", :not_found)
      when 408
        raise EnableBanking2Error.new("Request timeout from Enable Banking API", :timeout)
      when 422
        response_data = parse_response_body(response)
        raise EnableBanking2Error.new("Validation error from Enable Banking API: #{response.body}", :validation_error, response_data: response_data)
      when 429
        raise EnableBanking2Error.new("Rate limit exceeded. Please try again later.", :rate_limited)
      else
        raise EnableBanking2Error.new("Failed to fetch data: #{response.code} #{response.message} - #{response.body}", :fetch_failed)
      end
    end
    ...
    def parse_response_body(response)
      ...
      raise EnableBanking2Error.new("Failed to parse API response", :parse_error)
    end

    class EnableBanking2Error < StandardError
