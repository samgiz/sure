class EnableBanking2Item::Importer
  ...
  attr_reader :enable_banking2_item, :enable_banking2_provider

  def initialize(enable_banking2_item, enable_banking2_provider:)
    @enable_banking2_item = enable_banking2_item
    @enable_banking2_provider = enable_banking2_provider
  end

  def import
    unless enable_banking2_item.session_valid?
      enable_banking2_item.update!(status: :requires_update)
      return { success: false, error: I18n.t("enable_banking2_items.errors.session_invalid"), accounts_updated: 0, transactions_imported: 0 }
    end

    session_data = fetch_session_data
    unless session_data
      error_msg = @session_error || I18n.t("enable_banking2_items.errors.unexpected")
      return { success: false, error: error_msg, accounts_updated: 0, transactions_imported: 0 }
    end

    # Store raw payload
    begin
      enable_banking2_item.upsert_enable_banking2_snapshot!(session_data)
    rescue => e
      Rails.logger.error "EnableBanking2Item::Importer - Failed to store session snapshot: #{e.message}"
    end
    ...
    if session_data[:accounts].present?
      existing_uids = enable_banking2_item.enable_banking2_accounts
      ...
      session_data[:accounts].each do |account_data|
        ...
        begin
          ...
          Rails.logger.error "EnableBanking2Item::Importer - Failed to update account #{uid}: #{e.message}"
        end
      end
    end
    ...
    linked_accounts_query = enable_banking2_item.enable_banking2_accounts.joins(:account_provider).joins(:account).merge(Account.visible)

    linked_accounts_query.each do |enable_banking2_account|
      begin
        unless fetch_and_update_balance(enable_banking2_account)
          transactions_failed += 1
          # @sync_error already set in fetch_and_update_balance
          next
        end

        result = fetch_and_store_transactions(enable_banking2_account)
        if result[:success]
        ...
        transactions_failed += 1
        @sync_error = promote_session_invalid(@sync_error, handle_sync_error(e))
        Rails.logger.error "EnableBanking2Item::Importer - Failed to process account #{enable_banking2_account.uid}: #{e.message}"
      end
    end
    ...
    result[:error] = @sync_error || I18n.t("enable_banking2_items.errors.unexpected") if !result[:success]
    result
  end
  ...
  private
    ...
    def handle_sync_error(exception, session_level: false)
      ...
      provider_error = exceptions.find { |ex| ex.is_a?(Provider::EnableBanking2::EnableBanking2Error) }
      ...
      if session_level && provider_error && [ :unauthorized, :not_found ].include?(provider_error.error_type)
        enable_banking2_item.update!(status: :requires_update)
        return I18n.t("enable_banking2_items.errors.session_invalid")
      end

      is_network_error = exceptions.any? do |ex|
        NETWORK_ERRORS.any? { |err| ex.is_a?(err) } ||
          (ex.is_a?(Provider::EnableBanking2::EnableBanking2Error) && [ :request_failed, :timeout ].include?(ex.error_type))
      end

      if is_network_error
        I18n.t("enable_banking2_items.errors.network_unreachable")
      elsif provider_error
        I18n.t("enable_banking2_items.errors.api_error")
      else
        I18n.t("enable_banking2_items.errors.unexpected")
      end
    end

    def fetch_session_data
      session_data = enable_banking2_provider.get_session(session_id: enable_banking2_item.session_id)
      ...
      enable_banking2_item.reconcile_session_expiry!(session_data)
      session_data
    rescue Provider::EnableBanking2::EnableBanking2Error => e
      Rails.logger.error "EnableBanking2Item::Importer - Enable Banking API error: #{e.message}"
      @session_error = handle_sync_error(e, session_level: true)
      nil
    rescue => e
      Rails.logger.error "EnableBanking2Item::Importer - Unexpected error fetching session: #{e.class} - #{e.message}"
      @session_error = handle_sync_error(e, session_level: true)
      nil
    end
    ...
    def import_account(account_data)
      ...
      enable_banking2_account = find_enable_banking2_account_by_hash(uid)
      return unless enable_banking2_account

      enable_banking2_account.upsert_enable_banking2_snapshot!(account_data)
      enable_banking2_account.save!
    end

    def fetch_and_update_balance(enable_banking2_account)
      balance_data = enable_banking2_provider.get_account_balances(
        account_id: enable_banking2_account.api_account_id,
        psu_headers: enable_banking2_item.build_psu_headers
      )
      ...
      if balance.present?
        ...
        if amount.present?
          ...
          enable_banking2_account.update!(
            current_balance: parsed_amount,
            currency: currency.presence || enable_banking2_account.currency
          )
        end
      end
      true
      ...
      @sync_error = promote_session_invalid(@sync_error, handle_sync_error(e))
      Rails.logger.error "EnableBanking2Item::Importer - Error fetching balance for account #{enable_banking2_account.uid}: #{e.message}"
      false
    end

    def promote_session_invalid(existing, new)
      return new if existing.nil?
      return new if new == I18n.t("enable_banking2_items.errors.session_invalid")
      existing
    end
    ...
    def fetch_and_store_transactions(enable_banking2_account)
      start_date = determine_sync_start_date(enable_banking2_account)
      include_pending = include_pending?

      all_transactions = fetch_paginated_transactions(
        enable_banking2_account,
      ...
      if include_pending
        ...
        begin
          ...
          Rails.logger.warn "EnableBanking2Item::Importer - ASPSP does not support PDNG transaction status for account #{enable_banking2_account.uid}, skipping pending transactions. API error: #{api_error || e.message}"
        end
      end

      book_fingerprints = all_transactions
        .map { |tx| EnableBanking2Entry::Processor.compute_external_id(tx) }
        .compact.to_set
      ...
      pending_transactions.reject! do |tx|
        ...
        fp = EnableBanking2Entry::Processor.compute_external_id(tx_ia)
      ...
      existing_transactions = enable_banking2_account.raw_transactions_payload.to_a
      ...
      unless include_pending
        removed_pending = existing_transactions.reject! do |tx|
          ...
          tx.dig(:extra, :enable_banking2, :pending) || tx[:_pending]
        end
      end
      ...
      if all_transactions.any?
        ...
        #    diverge (enable_banking2_<ref> vs enable_banking2_<txn_id>) but the
        ...
        if include_pending
          removed_pending ||= existing_transactions.reject! do |tx|
            tx = tx.with_indifferent_access
            pending_flag = tx.dig(:extra, :enable_banking2, :pending) || tx[:_pending]
            next false unless pending_flag

            fp = EnableBanking2Entry::Processor.compute_external_id(tx)
        ...
        existing_ids = existing_transactions.map { |tx|
          EnableBanking2Entry::Processor.compute_external_id(tx)
        }.compact.to_set

        new_transactions = all_transactions.select do |tx|
          ext_id = EnableBanking2Entry::Processor.compute_external_id(tx)
          ext_id.present? && !existing_ids.include?(ext_id)
        end

        if new_transactions.any? || removed_pending
          enable_banking2_account.upsert_enable_banking2_transactions_snapshot!(existing_transactions + new_transactions)
        end
      elsif removed_pending
        enable_banking2_account.upsert_enable_banking2_transactions_snapshot!(
      ...
      Rails.logger.error "EnableBanking2Item::Importer - Error fetching transactions for account #{enable_banking2_account.uid}: #{e.message}"
      { success: false, transactions_count: 0, error: handle_sync_error(e) }
    rescue => e
      Rails.logger.error "EnableBanking2Item::Importer - Unexpected error fetching transactions for account #{enable_banking2_account.uid}: #{e.class} - #{e.message}"
      { success: false, transactions_count: 0, error: handle_sync_error(e) }
    end
    ...
    def deduplicate_api_transactions(transactions)
      ...
      if duplicates_removed > 0
        Rails.logger.info(
          "EnableBanking2Item::Importer - Removed #{duplicates_removed} content-level " \
    ...
    def fetch_paginated_transactions(enable_banking2_account, start_date:, transaction_status:, psu_headers: {})
      all_transactions = []
      ...
      loop do
        ...
        if page_count > MAX_PAGINATION_PAGES
          msg = "EnableBanking2Item::Importer - Pagination limit exceeded for account #{enable_banking2_account.uid} (status=#{transaction_status}). Stopped after #{MAX_PAGINATION_PAGES} pages."
          raise PaginationTruncatedError, msg
        end

        transactions_data = enable_banking2_provider.get_account_transactions(
          account_id: enable_banking2_account.api_account_id,
        ...
        if continuation_key.present? && continuation_key == previous_continuation_key
          msg = "EnableBanking2Item::Importer - Repeated continuation_key detected for account #{enable_banking2_account.uid} (status=#{transaction_status}). Breaking after #{page_count} pages."
          raise PaginationTruncatedError, msg
        end
    ...
    def find_enable_banking2_account_by_hash(hash_value)
      return nil if hash_value.blank?

      # First: exact uid match (primary identification_hash)
      account = enable_banking2_item.enable_banking2_accounts.find_by(uid: hash_value.to_s)
      return account if account

      # Second: search in identification_hashes array (PostgreSQL JSONB contains operator)
      enable_banking2_item.enable_banking2_accounts
    ...
    def sync_uids_from_accounts_data(accounts_data)
      ...
      accounts_data.each do |ad|
        ...
        eb_acc = find_enable_banking2_account_by_hash(identification_hash)
        next unless eb_acc
    ...
    def determine_sync_start_date(enable_banking2_account)
      has_stored_transactions = enable_banking2_account.raw_transactions_payload.to_a.any?

      # Use user-configured sync_start_date if set, otherwise default
      user_start_date = enable_banking2_item.sync_start_date

      if has_stored_transactions
        # For incremental syncs, get transactions from 7 days before last sync
        if enable_banking2_item.last_synced_at
          enable_banking2_item.last_synced_at.to_date - 7.days
        else
          30.days.ago.to_date
        end
      else
