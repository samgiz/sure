...
module EnableBanking2Item::Unlinking
  ...
  # - Detaches any AccountProvider links for each EnableBanking2Account
  # - Detaches Holdings that point at the AccountProvider links
  # Returns a per-account result payload for observability
  def unlink_all!(dry_run: false)
    results = []

    enable_banking2_accounts.find_each do |eba|
      links = AccountProvider.where(provider_type: "EnableBanking2Account", provider_id: eba.id).to_a
      ...
      begin
        ...
        Rails.logger.warn(
          "EnableBanking2Item Unlinker: failed to fully unlink EBA ##{eba.id} (links=#{link_ids.inspect}): #{e.class} - #{e.message}"
        )
