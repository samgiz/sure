...
class EnableBanking2Entry::Processor
  include CurrencyNormalizable

  # enable_banking2_transaction is the raw hash fetched from Enable Banking API
  ...
  def self.compute_external_id(raw_transaction_data)
    ...
    return "enable_banking2_#{id}" if id
    ...
    "enable_banking2_content_#{Digest::MD5.hexdigest(content)}"
  end

  def initialize(enable_banking2_transaction, enable_banking2_account:, import_adapter: nil)
    @enable_banking2_transaction = enable_banking2_transaction
  ...
  def process
    ...
    safe_id = self.class.compute_external_id(@enable_banking2_transaction) || "unknown"

    unless account.present?
      Rails.logger.warn "EnableBanking2Entry::Processor - No linked account for enable_banking2_account #{enable_banking2_account.id}, skipping transaction #{safe_id}"
      return nil
    end
    ...
    begin
      import_adapter.import_transaction(
        ...
        date: date,
        name: name,
        source: "enable_banking2",
        merchant: merchant,
        notes: notes,
        extra: extra
      )
    rescue ArgumentError => e
      Rails.logger.error "EnableBanking2Entry::Processor - Validation error for transaction #{safe_id}: #{e.message}"
      raise
    rescue ActiveRecord::RecordInvalid, ActiveRecord::RecordNotSaved => e
      Rails.logger.error "EnableBanking2Entry::Processor - Failed to save transaction #{safe_id}: #{e.message}"
      raise StandardError.new("Failed to import transaction: #{e.message}")
    rescue => e
      Rails.logger.error "EnableBanking2Entry::Processor - Unexpected error processing transaction #{safe_id}: #{e.class} - #{e.message}"
  ...
  private
    ...
    attr_reader :enable_banking2_transaction, :enable_banking2_account
    ...
    def data
      @data ||= enable_banking2_transaction.with_indifferent_access
    end
    ...
    def merchant
      ...
      @merchant ||= begin
        import_adapter.find_or_create_merchant(
          provider_merchant_id: "enable_banking2_merchant_#{merchant_id}",
          name: merchant_name,
          source: "enable_banking2"
        )
      rescue ActiveRecord::RecordInvalid => e
        Rails.logger.error "EnableBanking2Entry::Processor - Failed to create merchant '#{merchant_name}': #{e.message}"
        nil
      end
    end
    ...
    def extra
      ...
      eb.compact!
      eb.empty? ? nil : { enable_banking2: eb }
    end
