class EnableBanking2ItemsController < ApplicationController
  include EnableBanking2Items::MapsHelper
  before_action :set_enable_banking2_item, only: [ :update, :destroy, :sync, :select_bank, :authorize, :reauthorize, :setup_accounts, :complete_account_setup, :new_connection ]
  ...
  def new
    @enable_banking2_item = Current.family.enable_banking2_items.build
  end

  def create
    @enable_banking2_item = Current.family.enable_banking2_items.build(enable_banking2_item_params)
    @enable_banking2_item.name ||= "Enable Banking Connection"

    if @enable_banking2_item.save
      if turbo_frame_request?
        ...
        render turbo_stream: [
          turbo_stream.replace(
            "enable_banking2-providers-panel",
            partial: "settings/providers/enable_banking2_panel",
            locals: { enable_banking2_items: @enable_banking2_items }
          ),
      ...
      @error_message = @enable_banking2_item.errors.full_messages.join(", ")
  ...
  def update
    if @enable_banking2_item.update(enable_banking2_item_params)
      if turbo_frame_request?
  ...
  def select_bank
    unless @enable_banking2_item.credentials_configured?
    ...
    begin
      provider = @enable_banking2_item.enable_banking2_provider
      response = provider.get_aspsps(country: @enable_banking2_item.country_code)
  ...
  def authorize
    ...
    begin
      target_item = if params[:new_connection] == "true"
        Current.family.enable_banking2_items.create!(
          name: "Enable Banking Connection",
          country_code: @enable_banking2_item.country_code,
      ...
      redirect_url = target_item.begin_authorization!(
        ...
        redirect_url: enable_banking2_callback_url,
        state: target_item.id,
        psu_type: psu_type,
        language: language
      )

      safe_redirect_to_enable_banking2(
        redirect_url,
      ...
      if e.message.include?("REDIRECT_URI_NOT_ALLOWED")
        ...
        redirect_to settings_providers_path, alert: t(".redirect_uri_not_allowed",
          ...
          callback_url: enable_banking2_callback_url)
      else
  ...
  def callback
    ...
    # Find the enable_banking2_item by ID from state
    enable_banking2_item = Current.family.enable_banking2_items.find_by(id: state)

    unless enable_banking2_item.present?
    ...
    enable_banking2_item.update(last_psu_ip: request.remote_ip) if request.remote_ip.present?

    begin
      enable_banking2_item.complete_authorization(code: code)
  ...
  def new_connection
    ...
    redirect_to select_bank_enable_banking2_item_path(@enable_banking2_item, new_connection: true), data: { turbo_frame: "modal" }
  end
  ...
  def reauthorize
    begin
      ...
      redirect_url = @enable_banking2_item.begin_authorization!(
        redirect_url: enable_banking2_callback_url,
        state: @enable_banking2_item.id,
        language: language
      )
  ...
  def link_accounts
    ...
    enable_banking2_item = Current.family.enable_banking2_items.where.not(session_id: nil).first
    ...
    begin
      ActiveRecord::Base.transaction do
        selected_uids.each do |uid|
          enable_banking2_account = enable_banking2_item.enable_banking2_accounts.find_by(uid: uid)
          next unless enable_banking2_account

          # Check if already linked
          if enable_banking2_account.account_provider.present?
            already_linked_accounts << enable_banking2_account.name
            next
          end
          ...
          # Link account to enable_banking2_account via account_providers
          # Uses create! so any failure will rollback the entire transaction
          AccountProvider.create!(
            account: account,
            provider: enable_banking2_account
          )
    ...
    enable_banking2_item.sync_later if created_accounts.any?
  ...
  def setup_accounts
    @enable_banking2_accounts = @enable_banking2_item.enable_banking2_accounts
  ...
  def complete_account_setup
    ...
    if params[:sync_start_date].present?
      @enable_banking2_item.update!(sync_start_date: params[:sync_start_date])
    end

    created_count = 0
    skipped_count = 0

    account_types.each do |enable_banking2_account_id, selected_type|
      ...
      enable_banking2_account = @enable_banking2_item.enable_banking2_accounts.find(enable_banking2_account_id)
      selected_subtype = account_subtypes[enable_banking2_account_id]
      ...
      account = Account.create_from_enable_banking2_account(
    ...
    @enable_banking2_item.sync_later if created_count > 0
  ...
  def select_existing_account
    ...
    @available_enable_banking2_accounts = Current.family.enable_banking2_items
      .includes(:enable_banking2_accounts)
      .flat_map(&:enable_banking2_accounts)
  ...
  def link_existing_account
    @account = Current.family.accounts.find(params[:account_id])
    enable_banking2_account = EnableBanking2Account.find(params[:enable_banking2_account_id])
    ...
    if @account.account_providers.any? || @account.plaid_account_id.present? || @account.simplefin_account_id.present?
      flash[:alert] = t("enable_banking2_items.link_existing_account.errors.only_manual")
      if turbo_frame_request?
    ...
    unless enable_banking2_account.enable_banking2_item.present? &&
           Current.family.enable_banking2_items.include?(enable_banking2_account.enable_banking2_item)
      flash[:alert] = t("enable_banking2_items.link_existing_account.errors.invalid_enable_banking2_account")
      if turbo_frame_request?
    ...
    Account.transaction do
      enable_banking2_account.lock!

      # Upsert the AccountProvider mapping deterministically
      ap = AccountProvider.find_or_initialize_by(provider: enable_banking2_account)
    ...
    if turbo_frame_request?
      ...
      enable_banking2_account.reload
      item = enable_banking2_account.enable_banking2_item
      item.reload
      ...
      @enable_banking2_items = Current.family.enable_banking2_items.ordered.includes(:syncs)
      build_enable_banking2_maps_for(@enable_banking2_items)

      flash[:notice] = t("enable_banking2_items.link_existing_account.success")
      @account.reload
      ...
      render turbo_stream: [
        ...
        turbo_stream.replace(
          ...
          locals: { enable_banking2_item: item }
        ),
        turbo_stream.replace("modal", view_context.turbo_frame_tag("modal"))
      ] + Array(flash_notification_stream_items)
    else
      redirect_to accounts_path(cache_bust: SecureRandom.hex(6)), notice: t("enable_banking2_items.link_existing_account.success"), status: :see_other
    end
  end

  private

    def set_enable_banking2_item
      @enable_banking2_item = Current.family.enable_banking2_items.find(params[:id])
    end

    def enable_banking2_item_params
      params.require(:enable_banking2_item).permit(
        :name,
        :sync_start_date,
        :country_code,
        :application_id,
    ...
    def enable_banking2_callback_url
      helpers.enable_banking2_callback_url
    end
    ...
    def valid_enable_banking2_redirect_url?(url)
    ...
    def safe_redirect_to_enable_banking2(redirect_url, fallback_path:, fallback_alert:)
      if valid_enable_banking2_redirect_url?(redirect_url)
