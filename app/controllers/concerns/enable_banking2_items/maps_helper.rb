...
module EnableBanking2Items
  module MapsHelper
    extend ActiveSupport::Concern

    # Build per-item maps consumed by the enable_banking2_item partial.
    # Accepts a single EnableBanking2Item or a collection.
    def build_enable_banking2_maps_for(items)
      items = Array(items).compact
      return if items.empty?

      @enable_banking2_sync_stats_map ||= {}
      @enable_banking2_has_unlinked_map ||= {}
      @enable_banking2_unlinked_count_map ||= {}
      @enable_banking2_duplicate_only_map ||= {}
      @enable_banking2_show_relink_map ||= {}
      ...
      unlinked_counts = EnableBanking2Account
        .where(enable_banking2_item_id: items.map(&:id))
        .left_joins(:account, :account_provider)
        .where(accounts: { id: nil }, account_providers: { id: nil })
        .group(:enable_banking2_item_id)
        .count
      ...
      items.each do |item|
        ...
        @enable_banking2_sync_stats_map[item.id] = stats
        ...
        @enable_banking2_has_unlinked_map[item.id] = families_with_manuals.include?(item.family_id)

        # Count from batch query (defaults to 0 if not found)
        @enable_banking2_unlinked_count_map[item.id] = unlinked_counts[item.id] || 0

        # Whether all reported errors for this item are duplicate-account warnings
        @enable_banking2_duplicate_only_map[item.id] = compute_duplicate_only_flag(stats)
        ...
        begin
          unlinked_count = @enable_banking2_unlinked_count_map[item.id] || 0
          manuals_exist = @enable_banking2_has_unlinked_map[item.id]
          sfa_any = if item.enable_banking2_accounts.loaded?
            item.enable_banking2_accounts.any?
          else
            item.enable_banking2_accounts.exists?
          end
          @enable_banking2_show_relink_map[item.id] = (unlinked_count.to_i == 0 && manuals_exist && sfa_any)
        rescue StandardError => e
          Rails.logger.warn("Enable Banking card: CTA computation failed for item #{item.id}: #{e.class} - #{e.message}")
          @enable_banking2_show_relink_map[item.id] = false
        end
      end
    end
